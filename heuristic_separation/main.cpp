...
long s = 3;          // parameter s in s-club
		...
		try
		{
			if (where == GRB_CB_MIPSOL)
			{
				...
				if (induced_g.n >= 2)
				{
					...
					// define expressions to build the lazy cut
					...
					if (THETA + 1.5 < HS.size())
					{
						...
						long r = 1;
						Critical_set = solveMCDS(induced_HS, s);
						...
						{
							Critical_set_original.push_back(HS_original_index[Critical_set[i]]);
						}
						//find hereditary set
						long key;
						long c = 0;
						for (long i = 0; i < HS_original_index.size(); i++)
						{
							key = HS_original_index[i];
						...
						addLazy(Theta >= ((1 - Critical_vertices) * HS.size()) - Hereditary_vertices);
